...
```
kali@kali:~/CTFs/tryhackme/Brute It$ sudo nmap -A -sS -sC -sV -O 10.10.16.131
...
Checking the page source at `view-source:http://10.10.16.131/admin/` reveals a hint:

```html
<!-- Hey john, if you do not remember, the username is admin -->
```

Now brute force the admin panel login using Hydra:
...
After logging in to the admin panel, we're greeted with:
...
Downloading the RSA key and cracking its passphrase:
...
Now SSH in with the cracked passphrase:
...
`/bin/cat` can be run as root with no password — we can read any file on the system.
...
Reading `/etc/shadow` to grab the root hash:
...
Cracking the root hash with John the Ripper:
...
Brute It is a solid beginner room that walks through the full attack chain on a Linux machine — from port scanning to SSH access to privilege escalation. No fancy exploits, just methodical enumeration and the right tools used in the right order.

---

## Key Learnings

- **Always brute-force web directories** — `/admin` wasn't linked anywhere on the site, only gobuster found it.
- **Check page source for comments** — the dev left the username in an HTML comment. Never trust devs to clean up after themselves.
- **Hydra http-post-form syntax** — understanding how to capture the form parameters (`user=^USER^&pass=^PASS^`) and the failure string (`Username or password invalid`) is essential for web login bruteforcing.
- **ssh2john + John the Ripper** — RSA private keys can be passphrase-protected, and those passphrases can be cracked just like any other hash.
- **sudo -l is always the first thing to run** — misconfigured sudo permissions (`NOPASSWD: /bin/cat`) are extremely common and can trivially expose root-owned files.
- **`/bin/cat` as root = game over** — being able to cat `/etc/shadow` gives you every password hash on the system, which can then be cracked offline.
